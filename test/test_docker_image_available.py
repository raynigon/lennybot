...
class TestParseImage(unittest.TestCase):
    ...
    def test_parse_image_without_tag_separator(self):
        ...
        check = DockerImageAvailableCheck(
            "test-app", "0.2.3", "stable", self.config, self.container_config
        )
    ...
    def test_given_image_with_repo(self):
        ...
        check = DockerImageAvailableCheck(
            "test-app", "9.5.5", "9.5.6", self.config, self.container_config
        )
        check.check()
    ...
    def test_given_image_with_registry(self):
        ...
        check = DockerImageAvailableCheck(
            "test-app", "2.7.6", "2.7.7", self.config, self.container_config
        )
        check.check()
...
class TestAuthenticateImage(unittest.TestCase):
    def setUp(self) -> None:
        self.config = LennyBotCheckConfig()
        self.container_config = LennyBotConfigContainerConfig()
        self.docker_image_check = DockerImageAvailableCheck(
            "", "", "", self.config, self.container_config
        )
    ...
    def test_authenticate_on_registry_returns_access_token(self):
        ...
        access_token = self.docker_image_check._authenticate_on_registry(
            registry.name, header_value
        )
    ...
    def test_authenticate_on_registry_wrong_credentials(self):
        self.config._image_pattern = (
            "ghcr.io/brose-ebike/postgres-operator:v{{version}}"
        )
